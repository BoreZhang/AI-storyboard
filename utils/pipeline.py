...
class PhotoMakerStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def generate_image_with_layout(self, prompt, layout, character_images, **kwargs):
            layout_image = self.layout_generator.layout_to_image(layout, (kwargs.get('height', 512), kwargs.get('width', 512)))
            return self.__call__(prompt, input_id_images=character_images, layout_image=layout_image, **kwargs)
    ...
    def __call__(
        ...
        input_id_images: List[PipelineImageInput] = None,
        ...
        **kwargs,
        ...
        r"""
        调用管道进行生成时调用的函数。
        这里只讨论PhotoMaker引入的参数。
        对于StableDiffusionXLPipeline中先前参数的解释，请参阅https://github.com/huggingface/diffusers/blob/v0.25.0/src/diffusers/pipelines/stable_diffusion_xl/pipeline_stable_diffusion_xl.py

        Args:
            input_id_images (`PipelineImageInput`, *optional*):
                输入ID图像以与PhotoMaker一起使用。
            class_tokens_mask (`torch.LongTensor`, *optional*):
                预生成的类token。当提前提供`prompt_embeds`参数时，需要预先准备`class_tokens_mask`以标记类词的位置。
            prompt_embeds_text_only (`torch.FloatTensor`, *optional*):
                预生成的文本嵌入。可用于轻松调整文本输入，例如提示加权。如果未提供，将从`prompt`输入参数生成文本嵌入。
            pooled_prompt_embeds_text_only (`torch.FloatTensor`, *optional*):
                预生成的池化文本嵌入。可用于轻松调整文本输入，例如提示加权。如果未提供，将从`prompt`输入参数生成池化文本嵌入。

        Returns:
            [`~pipelines.stable_diffusion_xl.StableDiffusionXLPipelineOutput`] 或 `tuple`:
            如果`return_dict`为True，则返回[`~pipelines.stable_diffusion_xl.StableDiffusionXLPipelineOutput`]，否则返回`tuple`。返回元组时，第一个元素是生成的图像列表。
        """
        # 0. 默认高度和宽度为unet
        ...
        if prompt_embeds is not None and class_tokens_mask is None:
            raise ValueError(
                "如果提供了`prompt_embeds`，则还必须传递`class_tokens_mask`。确保从用于生成`prompt_embeds`的相同tokenizer生成`class_tokens_mask`。"
        ...
        if input_id_images is not None:
            id_pixel_values = []
            for id_image in input_id_images:
                if not isinstance(id_image[0], torch.Tensor):
                    id_pixel_value = self.id_image_processor(id_image, return_tensors="pt").pixel_values
                id_pixel_values.append(id_pixel_value)
            id_pixel_values = torch.cat(id_pixel_values, dim=0).to(device=device, dtype=dtype)
        ...
        # 这里`guidance_scale`定义类似于Imagen论文中方程（2）的指导权重`w`： https://arxiv.org/pdf/2205.11487.pdf 。`guidance_scale = 1`
        # 对应于不进行分类器自由指导。
        ...
        if isinstance(prompt, list):
            ...
            # 9. 准备额外步骤kwargs。TODO: 逻辑应理想地移出管道
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance and guidance_rescale > 0.0:
                    # 基于https://arxiv.org/pdf/2305.08891.pdf中的3.4
        ...
        if not return_dict:
            return (image,)
        # 解析提示词，更新主题管理器
        self.subject_manager.set_scene(prompt)
        context = self.subject_manager.get_context()

        # 生成布局
        layout = self.layout_generator.generate_layout(context, (kwargs.get('height', 512), kwargs.get('width', 512)))

        # 优化布局
        suggestions = self.supervisor.refine_layout(layout, context)
        refined_layout = self.layout_generator.generate_layout(context + "\n" + suggestions, (kwargs.get('height', 512), kwargs.get('width', 512)))
